...
const EmployeeKPIReport = ({ employeeId }) => {
  ...
  try {
    ...
    setData(kpiData);
    // If employee name is part of kpiData, set it here:
    // setEmployeeName(kpiData.employeeName || '');
  } catch (error) {
    console.error('Failed to fetch KPI data:', error.response?.data || error.message);
        ...
        <tbody>
          ...
          {data.map((kpi) => (
            <tr key={`${kpi.KRA}-${kpi.KPI}`}>
              ...
              <td className="border px-3 py-1 text-center align-top">{kpi.weitage}</td>
              ...
              <td className="border px-3 py-1 text-center">{kpi.Q1_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.Q2_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.Q3_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.Q4_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.H1_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.H2_Average ?? '-'}</td>
              <td className="border px-3 py-1 text-center">{kpi.Year_Average ?? '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
